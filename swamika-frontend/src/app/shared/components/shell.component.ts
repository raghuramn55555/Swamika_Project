import { Component, signal, inject, HostListener, ChangeDetectionStrategy } from '@angular/core';
...
export class ShellComponent {
  ...
  get title() {
    return TITLES[this.activeView()][0];
  }
  get subtitle() {
    return TITLES[this.activeView()][1];
  }
  ...
  proposeTeam() {
    this.teamProposed = true;
  }

  teamRows = [
    {
      ref: 'CV-1042',
      contrib: 'Spring Boot, PostgreSQL, REST',
      cov: '+41%',
      badge: 'ready',
      label: 'Locked',
    },
    ...
    {
      ref: 'CV-1156',
      contrib: 'French, client-facing delivery',
      cov: '+6%',
      badge: 'warn',
      label: 'Gap: PostgreSQL depth',
    },
  ];
  ...
  handleFiles(files: FileList | File[]): void {
    ...
    if (list.length === 1) {
      ...
      this.cvSvc.uploadOne(list[0]).subscribe({
        next: (prog) => {
          this.uploadQueue.update((q) => q.map((r) => (r.filename === prog.filename ? prog : r)));
        },
        ...
        next: (bulk) => {
          ...
          for (const d of bulk.duplicates) {
            byName.set(d.filename, {
              ...
              progress: 100,
              status: 'error',
              errorMsg: 'Duplicate — already in library.',
            });
          }
          ...
          this.uploadQueue.update((q) => q.map((r) => byName.get(r.filename) ?? r));
        },
        error: (err: Error) => {
          this.uploadError.set(err.message);
          this.uploadQueue.update((q) =>
            q.map((r) =>
              list.some((f) => f.name === r.filename)
  ...
  uploads = [
    {
      name: 'A_Rahman_CV.pdf',
      badge: 'ready',
      badgeText: 'Ready',
      meta: '1.2 MB · structuring complete',
      barW: '100%',
      barColor: '',
    },
    {
      name: 'K_Ito_Resume.docx',
      badge: 'processing',
      badgeText: 'Embedding',
      meta: '640 KB · generating vectors',
      barW: '64%',
      barColor: '',
    },
    {
      name: 'scan_0091.pdf',
      badge: 'warn',
      badgeText: 'OCR required',
      meta: 'Insufficient extractable text',
      barW: '100%',
      barColor: 'var(--amber)',
    },
    {
      name: 'CV_corrupted.docx',
      badge: 'failed',
      badgeText: 'Failed',
      meta: 'Unreadable file — retry or replace',
      barW: '100%',
      barColor: 'var(--red)',
    },
  ];
  ...
  recentSearches = [
    {
      query: 'Senior Java / Spring Boot engineer',
      mode: 'Vacancy',
      count: 34,
      blind: 'On',
      ago: '12 min ago',
    ...
    {
      query: '"React developer with AWS and CI/CD"',
